...
- **Monitor 10026** (`users.monitor_list_<slot>`):  
...
1. **Auto-entry (this trade)**  
   ...
   - `trade_strategy` = `get_trade_strategy()` (reads `strategy` from `users.monitor_list_<slot>` for that same `MONITOR_ID`).
...
So at the time this trade was created, **monitor 10026 in `users.monitor_list_<slot>` most likely had**:
...
1. **History of monitor 10026**  
   If you have history (e.g. audit table, backups, or logs) for `users.monitor_list_<slot>` for id 10026, confirm whether it was ever ETH / hourly / Hourly HTC.
