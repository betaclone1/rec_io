...
- **BTC Exp Scalp cutout:** Ladder-notify → full Exp Scalp scan cadence is unchanged. Cutout may share the cheap status helper; do not slow cutout live tick → eval. **Status writes** (`periodic_status_sync` / cheap status) use the same membership as fire: cutout only BTC 15m Expiration Scalp; unified excludes those monitors.
