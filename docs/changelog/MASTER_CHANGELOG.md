...
- [x] Apply migration: `PYTHONPATH=$(pwd) venv/bin/python scripts/db/run_migration.py up 20260918_1400_short_horizon_deltas_adverse_15s`
- [x] Regenerate supervisor config and full restart:
  `cd /opt/rec_io_server && scripts/MASTER_RESTART.sh`
- [x] Verify: health 3000/8001; `active_trade_supervisor_0001` / `trade_manager_0001` / `trade_executor_0001` / AES exp-scalp cutout RUNNING; no ATS failsafe deadlock; Exp Scalp settings show Adverse 15s control
- [x] Record release in DB: `PYTHONPATH=$(pwd) venv/bin/python scripts/ops/record_system_version.py --version 3.12.9`
...
- [x] Confirm codebase changes (pull latest on production):
  `cd /opt/rec_io_server && git fetch && git checkout main && git pull --ff-only origin main`
- [x] Apply migration: `PYTHONPATH=$(pwd) venv/bin/python scripts/db/run_migration.py up 20260908_1945_position_risk_monitor_settings`
- [x] Regenerate supervisor config and full restart:
  `cd /opt/rec_io_server && scripts/MASTER_RESTART.sh`
- [x] Verify: health 3000/8001; `supervisorctl status position_risk_engine` is RUNNING; High Water Scalp ATS still RUNNING; no PRE refuse-to-start in `logs/position_risk_engine.err.log`
- [x] Record release in DB: `PYTHONPATH=$(pwd) venv/bin/python scripts/ops/record_system_version.py --version 3.12.7`
...
- [x] Regenerate supervisor config and full restart:
  `cd /opt/rec_io_server && scripts/MASTER_RESTART.sh`
- [x] Verify: health 3000/8001; `/api/cycle_recon/health` returns 403 in production (disabled); Backtester not required on prod; trading services RUNNING
- [x] Record release in DB: `PYTHONPATH=$(pwd) venv/bin/python scripts/ops/record_system_version.py --version 3.12.6`
...
- [x] Regenerate supervisor config and full restart:
  `cd /opt/rec_io_server && scripts/MASTER_RESTART.sh`
- [x] Verify: health 3000/8001; monitor settings save `weekend_adjustment`; MM weekend scheduler running
- [x] Record release in DB: `PYTHONPATH=$(pwd) venv/bin/python scripts/ops/record_system_version.py --version 3.12.3`
