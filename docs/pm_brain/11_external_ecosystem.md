...
- **Digital Ocean (for future DO specialist agent):** API: https://docs.digitalocean.com/reference/api/api-reference/ . Managed PostgreSQL: GET `https://api.digitalocean.com/v2/databases/{database_cluster_uuid}` returns connection object (host, port, user, password, db, ssl). doctl: `doctl databases connection <cluster-id>`. Public or VPC; firewall via API. PyDo/Godo for automation.
