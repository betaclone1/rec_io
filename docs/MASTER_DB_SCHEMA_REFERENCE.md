...
- **Check (weekend adjustment):** `{table}_weekend_adjustment_chk` — `weekend_adjustment` ∈ `none`, `paper_only`, `reduce_position_50`, `reduce_position_25`, `probability_adjustment_10`, `probability_adjustment_25`. Migration `20260831_1430_monitor_weekend_adjustment`.
...
| `momentum_5s_avg` | `numeric(10,4)` | YES | - | |
| `order_id` | `text` | YES | - | |
| `order_id_open` | `text` | YES | - | Active confirm pointer for the current open leg (Kalshi order UUID). Equality wakes/indexes use this scalar. |
| `order_id_close` | `text` | YES | - | Active confirm pointer for the current close leg. |
| `order_ids_open` | `ARRAY` | NO | `'{}'::text[]` | Append-only membership of **filled** open-leg order ids (`TEXT[]`). Never cleared by zero-fill top-ups; failed attempt ids are not appended. Migration `20260731_0025_trades_order_ids_open_close_arrays`. |
| `order_ids_close` | `ARRAY` | NO | `'{}'::text[]` | Append-only membership of **filled** close-leg order ids (`TEXT[]`). Same migration. |
...
| `order_id_open` | text |
| `order_id_close` | text |
| `order_ids_open` | ARRAY |
| `order_ids_close` | ARRAY |
| `time_in_force` | text |
| `order_type` | text |
