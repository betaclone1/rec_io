...
| Layer | Purpose |
|-------|---------|
| **Backend** (`backend/`) | Python services: FastAPI main app, trade manager, trade executor, **per-tenant** unified auto_entry / active_trade supervisors (plus watchdogs for Kalshi, price, strike tables), account sync, health monitors. All use a single DB config and port manifest. |
...
- **Trading:** User/monitor config in `users` / `users_NNNN.monitor_list_*` → **one** `auto_entry_supervisor` per trading tenant (`unified`) → trade_manager (lifecycle) → trade_executor (Kalshi orders). Open positions monitored by **one** `active_trade_supervisor` per tenant (`unified`). Fills/positions synced back via kalshi_account_sync.
...
- **Per-tenant tradeflow supervisors:** `generate_unified_supervisor_config.py` creates one `auto_entry_supervisor_<NNNN>` and one `active_trade_supervisor_<NNNN>` (argv `unified`) per trading tenant with active monitors — not one OS process per monitor. `monitor_manager` / main_app sync supervisord when monitors are created or activated.
