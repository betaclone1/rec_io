...
- **Trading:** User/monitor config in `users.monitor_list_<slot>` → auto_entry_supervisor (entry logic) → trade_manager (lifecycle) → trade_executor (Kalshi orders). Fills/positions synced back via kalshi_account_sync.
