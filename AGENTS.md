...
**Production deploy / mutate (non-negotiable — PRIME):** Never change production without an **explicit instruction in this chat** (no scp of code, no prod `git pull`, no apply-update/deploy skills, no supervisord regen, no restart-to-pick-up-new-code unless ordered). Do not infer from implementation work, CPU fixes, or prior deploys. Local code only until the user says to deploy. Read-only prod inspection is fine when the task needs it. See `.cursor/rules/01-core-operating-law.mdc`.
