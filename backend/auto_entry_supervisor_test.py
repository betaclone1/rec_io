...
def get_monitor_identifier():
    """Extract monitor identifier from command line arguments"""
    # Check command line arguments first
    if len(sys.argv) > 1:
        monitor_arg = sys.argv[1]
        if '_' in monitor_arg and monitor_arg.count('_') == 1:
            return monitor_arg
...
def get_monitor_symbol():
    ...
    try:
        ...
        print(f"[AUTO_ENTRY_SUPERVISOR] ❌ Error getting monitor symbol: {e}, defaulting to BTC")
        return "BTC"  # Default fallback
...
# Track logged strikes with timestamps to prevent duplicate logging within 15 seconds
            ...
            if monitor_result:
                ...
                if strategy_result:
                    settings = {
                        ...
                        "watchlist_min_volume": strategy_result[10],  # From monitor min_volume
...
def trigger_auto_entry_trade(strike_data):
    ...
    try:
        ...
        # TEST MODE: Comment out actual trade sending
        # response = requests.post(url, json=trade_payload, timeout=10)
        
        # TEST MODE: Simulate successful trade (no logging)
        return True
        
        # ACTUAL RESPONSE HANDLING CODE (COMMENTED OUT FOR TESTING):
...
def check_auto_entry_conditions():
    ...
    try:
        ...
        # TEST MODE: Ignore auto trade status - always treat as enabled
        auto_trade_enabled = True  # Force enabled for testing
        ...
        for i, strike in enumerate(watchlist_data["strikes"]):
            try:
                ...
                strike_data = {
                    ...
                    'buy_price': buy_price,
                    'probability': prob,
                    'buffer': strike.get('buffer'),
                    'buffer_pct': strike.get('buffer_pct')
                }
                ...
                # STEP 8: Log trade detection in clean format (only once)
                log_trade_detection(strike_data, watchlist_data)
                
                # STEP 9: Trigger the trade (silent in test mode)
...
def log_trade_detection(strike_data, strike_table_data):
    """Log trade detection in clean format - only if 15+ seconds since last log for this strike"""
    from datetime import datetime
    from zoneinfo import ZoneInfo
    
    strike_key = f"{strike_data.get('strike')}-{strike_data.get('side')}"
    
    # Check if we've logged this strike recently (within 15 seconds)
    current_time = datetime.now(ZoneInfo("America/New_York")).timestamp()
    if strike_key in logged_strikes:
        last_log_time = logged_strikes[strike_key]
        if current_time - last_log_time < 15:  # Less than 15 seconds ago
            return
    
    # Update timestamp for this strike
    logged_strikes[strike_key] = current_time
    
    # Get additional data from strike table
    current_ttc = strike_table_data.get("ttc", 0)
    current_momentum = auto_entry_indicator_state.get("current_momentum", "N/A")
    
    # Get buffer value - try both buffer and buffer_pct
    buffer_value = strike_data.get('buffer')
    if buffer_value is None:
        buffer_value = strike_data.get('buffer_pct')
    if buffer_value is None:
        buffer_value = "N/A"
    
    # Buffer values now included in strike_data
    
    # Format the clean log line
    clean_log_line = f"[TRADE DETECTED] Strike: {strike_data.get('strike')} | Side: {strike_data.get('side').upper()} | TTC: {current_ttc}s | Buffer: {buffer_value} | Momentum: {current_momentum}% | Probability: {strike_data.get('probability')}%"
    
    # Write to separate clean log file
    try:
        clean_log_path = f"/opt/rec_io_server/logs/trade_detections_{MONITOR_IDENTIFIER}.log"
        with open(clean_log_path, "a", encoding="utf-8") as f:
            timestamp = datetime.now(ZoneInfo("America/New_York")).strftime("%Y-%m-%d %H:%M:%S")
            f.write(f"[{timestamp}] {clean_log_line}\n")
    except Exception as e:
        print(f"Error writing to clean log file: {e}")
...
def spike_alert_settings():
    ...
    try:
        if request.method == 'GET':
            ...
            data = request.json
            # LEGACY REMOVED: No longer reading from JSON file - using database only
            settings = {}
            ...
            # LEGACY REMOVED: No longer saving to JSON file - using database only
...
def start_event_driven_supervisor():
    ...
    def start_http_server():
        try:
            ...
            port = 8015  # Use different port for test version
            log(f"🌐 Starting TEST HTTP server on {host}:{port}")
