...
class ProbabilityLookupTableManager:
    ...
    def validate_table_structure(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            required_columns = [
                "ttc_seconds", "buffer_points", "percentile_bucket", 
            ...
            for col in validation_result["table_info"]["columns"]:
                ...
                elif col["name"] == "percentile_bucket" and col["type"] not in ["integer", "bigint"]:
                    validation_result["errors"].append(f"percentile_bucket should be integer, got {col['type']}")
            ...
            # Check percentile bucket range
            cursor.execute(f"SELECT MIN(percentile_bucket), MAX(percentile_bucket) FROM {self.analytics_schema}.{table_name}")
            min_percentile, max_percentile = cursor.fetchone()
            validation_result["table_info"]["percentile_range"] = {"min": min_percentile, "max": max_percentile}
            
            if min_percentile < -99 or max_percentile > 99:
                validation_result["warnings"].append(f"Percentile range is {min_percentile}-{max_percentile}, expected -99 to 99")
