...
3. **Run verify-local** — Execute the full verification workflow **exactly as the verify-local command specifies** (`.cursor/commands/verify-local.md` and verify-local skill; workflow steps in `.cursor/pm/VERIFY_COMMAND.md`). That means: Health, Supervisor, Logs, Summary, and **the required status block at the end** (the `---` / `## VERIFY STATUS` / status line with icon and text, e.g. `✅ All good`; do not report "All good" if there are errors in logs). Do not omit any step.

Ports and log paths: .cursor/pm/brain/02_services_ports.md and project `logs/` directory. Full verify spec: .cursor/pm/VERIFY_COMMAND.md and .cursor/commands/verify-local.md.
