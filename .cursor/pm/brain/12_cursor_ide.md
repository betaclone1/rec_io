...
### Google API integrations (Drive, Docs, Sheets, Gmail, Calendar)

- **Setup:** `.cursor/pm/GOOGLE_DRIVE_MCP_SETUP.md`. Credentials: `.cursor/gcp-oauth.keys.json`, `.cursor/gdrive-server-credentials.json`. MCP entry in `.cursor/mcp.json` (env: GDRIVE_OAUTH_PATH, GDRIVE_CREDENTIALS_PATH).
- **Enabled APIs (GCP):** Drive (MCP + scripts), Docs (create-doc.js), Sheets (create-account-history-sheet.js). Gmail and Calendar: **google-workspace** MCP (`@presto-ai/google-workspace-mcp`) in `.cursor/mcp.json`; one-time OAuth on first use, credentials in `~/.config/google-workspace-mcp/`.
- **Read (MCP):** **search** tool (query Drive); **gdrive:///<file_id>** resources (Docs→Markdown, Sheets→CSV, etc.). REC_IO folder, Cursor subfolder.
- **Write — Docs:** `scripts/gdrive/create-doc.js` — create Google Doc or .txt (`--folder "Cursor" "Title" ["body"]`, `--text-file` only for plain text), delete by ID (`delete FILE_ID [FILE_ID...]`), auth (`auth` / `auth --code "URL"`). **When the user says "write a doc" or "create a document":** use without `--text-file` so the result is a Google Doc.
- **Write — Spreadsheets:** `scripts/gdrive/create-account-history-sheet.js` — exports `users.account_history_0001` to a new Google Sheet in REC_IO/Cursor. Uses `export_account_history.py` for DB export. Requires Sheets API and scope `spreadsheets`.
- **Gmail & Calendar (MCP):** **google-workspace** server — Gmail (search, read, send, drafts), Calendar (list calendars/events, create/update/delete events). First use triggers OAuth; tokens in `~/.config/google-workspace-mcp/`. See GOOGLE_DRIVE_MCP_SETUP.md.
- **Heavy use:** GOOGLE_DRIVE_MCP_SETUP.md and scripts/gdrive/README.md. **Default location:** REC_IO / Cursor when the user says they've put something on Drive for the agent.

### MCP recommendations (investigated for team)

**Currently enabled (this project):** G Drive (read + script write), **google-workspace** (Gmail + Calendar), Discord Kalshi, context7 (docs), cursor-ide-browser (browser automation).

**Useful to consider adding (from registry/directories):**
- **GitHub MCP** — Repo/issue/PR access, code search, Actions; useful for release and PR workflow.
- **Postgres MCP** — Read-only DB access from agent (schema introspection, safe queries); complements @db and scripts.
- **Fetch / web** — Official MCP Fetch or similar for web content; we have browser MCP for live interaction.
- **Linear / Notion / Slack MCP** — If we use them for PM or comms; Slack already used (see ORG_CHART).
- **Docker MCP** — If we containerize services or run stacks via Compose.
- **Official reference servers (modelcontextprotocol/servers):** Filesystem (secure file ops), Git (repo read/write), Memory (persistent knowledge graph), Time (timezone). Evaluate per need.

**Directories:** registry.modelcontextprotocol.io; Awesome MCP (mcp-awesome.com); Cursor Marketplace. Re-evaluate periodically.
