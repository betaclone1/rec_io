...
| (PM doc) PROD_MAINTENANCE_FROM_LOCAL | Primary prod update path: from local via /apply-update-from-local (SSH to prod). /apply-update only when agent is on prod. Verify-production and daily-briefing use SSH from local. Migrations reach prod only via git (no SCP). |
