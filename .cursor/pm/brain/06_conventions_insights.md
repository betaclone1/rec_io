...
## DB changes: migrations vs one-off commands (mandatory)

**Do not litter the project and repository with one-off scripts that only update the DB.** For simple one-off DB changes (e.g. add a column once, one-time backfill, trivial DDL that will not be reverted), **do not create new migration files.** Instead, document the exact command or SQL in the MASTER_CHANGELOG Production checklist (e.g. "Run on prod: `psql ... -c \"ALTER TABLE users.foo ADD COLUMN bar TEXT;\"`" or a one-liner Python). The apply-update workflow runs those commands on prod via SSH. Use **migration files** (`scripts/migrations/*.sql` + `run_migration.py`) only for schema changes that are part of the versioned, reversible schema (must run on every env, need rollback). When in doubt: if it's a one-off and can be done with a simple command, put the command in the checklist, not in a new file.
