...
install_first_boot_sanitization() {
    ...
    chmod +x scripts/first_boot_sanitize.sh
...
SERVICE_EOF
    ...
    ./scripts/setup_welcome_message.sh
...
- `/opt/rec_io/scripts/first_boot_sanitize.sh` - Sanitization script
...
test_service() {
    ...
    if [[ -x "scripts/first_boot_sanitize.sh" ]]; then
