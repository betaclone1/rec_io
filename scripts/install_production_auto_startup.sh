...
install_auto_startup() {
    ...
    if [[ -f "/opt/rec_io_server/scripts/install_auto_startup_service.sh" ]]; then
        chmod +x /opt/rec_io_server/scripts/install_auto_startup_service.sh
        /opt/rec_io_server/scripts/install_auto_startup_service.sh
        print_success "Auto startup service installed successfully"
    else
        print_error "Auto startup service script not found"
        print_status "Please ensure the script is in /opt/rec_io_server/scripts/"
        exit 1
    fi
}
