-- Per-tenant trades: string snapshot of monitor loss_prevention at insert time.
-- Monitor / strategy defaults: rename legacy win-streak value one_contract -> win_streak_one_contract.
...
BEGIN
  ...
  IF to_regclass('system.strategy_list_default') IS NOT NULL THEN
    ...
    SET loss_prevention = 'win_streak_one_contract'
    WHERE loss_prevention IS NOT NULL
      AND lower(trim(loss_prevention::text)) = 'one_contract';
  END IF;
END $$;
