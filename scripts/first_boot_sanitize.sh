...
EOF
    ...
    if [[ -f "/opt/rec_io/scripts/install_auto_startup_service.sh" ]]; then
        chmod +x /opt/rec_io/scripts/install_auto_startup_service.sh
        /opt/rec_io/scripts/install_auto_startup_service.sh
    ...
    print_warning "Run: ./scripts/collaborator_setup.sh"
    echo ""
...
[Service]
Type=oneshot
ExecStart=/opt/rec_io/scripts/first_boot_sanitize.sh
RemainAfterExit=yes
