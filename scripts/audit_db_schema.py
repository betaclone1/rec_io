...
Usage: PYTHONPATH=$(pwd) venv/bin/python scripts/audit_db_schema.py
...
def normalize_type_for_compare(t):
    """Normalize type string for comparison (TIMESTAMP WITH TIME ZONE -> TIMESTAMPTZ, etc.)."""
    t = t.upper().strip()
    if 'TIMESTAMP WITH TIME ZONE' in t or t == 'TIMESTAMPTZ':
        return 'TIMESTAMPTZ'
    if 'TIMESTAMP WITHOUT TIME ZONE' in t or t == 'TIMESTAMP':
        return 'TIMESTAMP'
    if 'TIME WITHOUT TIME ZONE' in t:
        return 'TIME'
