#!/usr/bin/env python3
"""
Final one-time backfill script to populate bankroll column in trades table
Handles edge cases and ensures ALL trades get bankroll values
"""
...
def backfill_trades_bankroll_final():
    """Backfill bankroll column in trades table with complete coverage"""
    ...
    try:
        ...
        print("🔍 Starting final bankroll backfill for trades table...")
        ...
        # Get the earliest available balance record as fallback
        cursor.execute("""
            SELECT balance, created_at FROM users.account_balance_0001 
            WHERE balance IS NOT NULL 
            ORDER BY created_at ASC 
            LIMIT 1
        """)
        earliest_balance = cursor.fetchone()
        
        if earliest_balance:
            fallback_balance = earliest_balance[0]
            fallback_time = earliest_balance[1]
            print(f"🔄 Using fallback balance: {fallback_balance} (from {fallback_time})")
        else:
            print("❌ No fallback balance available")
            return
        ...
        for trade in trades:
            ...
            if result and result[0] is not None:
                ...
                cursor.execute("""
                    SELECT balance 
                    FROM users.account_balance_0001 
                    WHERE created_at <= %s AND balance IS NOT NULL
                    ORDER BY created_at DESC 
                    LIMIT 1
                """, (trade_created_at,))
                ...
                if result and result[0] is not None:
                    ...
                    # Use fallback balance for trades before any account balance records
                    bankroll_value = fallback_balance
                    source = f"fallback_balance (earliest: {fallback_time})"
...
if __name__ == "__main__":
    backfill_trades_bankroll_final()
